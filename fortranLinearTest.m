...
main = string(datetime('now','Format','M-d-y@HH.mm.ss'))+"fortranLinearTest";
mkdir(main);
tstart = tic;
...
for i = 1:1000
    r = rand*100;
    cost = cos(rand*2*pi);
    v = V_2dlinear(r,cost,rlist,thetalist,V_2dlist);
    w = V_2d(r,cost, nr,nt,rori,coef,fact,torib);
    % if ((abs(v-w)/abs(w) > 1e-3 && r < rlist(end)) || (v < w && r >= rlist(end)))
        fprintf("V_2dlinear(%.20f,%.20f)\n= %.20f.\n",r,cost,v);
        fprintf("V_2d(%.20f,%.20f)\n= %.20f.\n",r,cost,w);
        if r >= rlist(end)
            fprintf("The function potararlinear gives a linear interpolation when r < 50 angstroms,\n" + ...
                "but outputs a positive number when r > 50 angstroms.\n" + ...
                "The clusters are less than 20 angstroms wide, so this doesn't affect results.\n");
        else
            fprintf("Relative error is %.10f percent (%.10f wavenumbers).\n",(abs(v-w)/abs(w)*100),abs(v-w));
        end
        fprintf("\n");
    % end
end
for i = 1:1000
    ...
    w = potarar(r, cktocm,ekelvin,A,alp,bet,C6,C8,C10,D,F);
    % if ((abs(v-w)/abs(w) > 1e-3 && r < rlist(end)) || (v < w && r >= rlist(end)))
        fprintf("potararlinear(%.20f)\n= %.20f.\n",r,v);
        fprintf("potarar(%.20f)\n= %.20f.\n",r,w);
        if r >= rlist(end)
            fprintf("The function potararlinear gives a linear interpolation when r < 50 angstroms,\n" + ...
                "but outputs a positive number when r > 50 angstroms.\n" + ...
